/* ================================================================
   METTLESTATE × EA FC MOBILE — players.js
...
// Marks a player as needing a catch-up game (behind on matches played vs the rest
// of the league). Surfaced on the admin table, the player modal, and pushed to the
// public leaderboard so the profile gauge can show a "catch-up" status.
function toggleCatchup(username) {
  const p = getPlayer(username);
  if (!p) return;
  p.catchup = !p.catchup;
  saveData();
  sendDiscordWebhook({ type: 'catchup', player: p.name, catchup: p.catchup });
  toast(`${p.name} ${p.catchup ? 'flagged for catch-up' : 'cleared from catch-up'}.`, p.catchup ? 'info' : 'success');
}
    ...
    return `
      <tr class="${suspClass}" style="animation-delay:${i * 0.03}s">
        ...
        <td class="player-info-cell">
          <div class="player-name-main">${esc(p.name)}${p.suspended ? '<span class="susp-badge">SUSP</span>' : ''}${p.catchup ? '<span class="catchup-badge">CATCH-UP</span>' : ''}</div>
        ...
        <td class="action-cell">
          <button class="btn-icon btn-catchup ${p.catchup ? 'active' : ''}" onclick="toggleCatchup('${esc(p.username)}')" title="${p.catchup ? 'Clear catch-up flag' : 'Flag for catch-up'}">
  ...
  content.innerHTML = `
    <div class="modal-header">
      ...
      <div>
        ...
        ${p.catchup ? '<div class="modal-catchup-badge">CATCH-UP NEEDED</div>' : ''}
      </div>
    </div>
    <div class="modal-stats-grid">
      ...
      <div class="modal-stat"><span class="ms-val">${p.gf||0}</span><span class="ms-lbl">Goals For</span></div>
      <div class="modal-stat"><span class="ms-val">${p.ga||0}</span><span class="ms-lbl">Goals Against</span></div>
      <div class="modal-stat"><span class="ms-val ${gd >= 0 ? 'gd-pos' : 'gd-neg'}">${gd > 0 ? '+'+gd : gd}</span><span class="ms-lbl">Goal Diff</span></div>
    </div>

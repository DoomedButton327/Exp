...
async function pushPublicLeaderboard() {
  ...
  // Trim results down to just the fields the public site needs (no uid/editedAt
  // clutter) — but DO keep the evidence image so the profile view can show proof.
  ...
  // Today's unplayed fixtures (not postponed) — feeds the public "Today" tab
  // and each player's status gauge.
  const fixturesToday = (State.fixtures || [])
    .filter(f => f.scheduledDate === today && !f.postponedBy)
    .map(f => ({ home: f.home, away: f.away, date: f.scheduledDate }));

  // Currently-postponed fixtures, regardless of original date — the "blue
  // section" on the public site.
  const fixturesPostponed = (State.fixtures || [])
    .filter(f => !!f.postponedBy)
    .map(f => ({ home: f.home, away: f.away, date: f.scheduledDate, postponedBy: f.postponedBy }));

  const data = {
    players: sorted,
    results,
    fixtures: { today: fixturesToday, postponed: fixturesPostponed },
